...
# [수정] 탭 4: 학생별 종합 성취 분석 (가독성 최적화)
with tab4:
    st.header("🎓 학생별 종합 성취 분석")

    # 1. 모든 학생을 한 그래프에 표시 (출석률 vs 학습량 vs 성취도)
    ...
    fig1 = px.scatter(
        ...
        color="학생명",          # 학생별 고유 색상
        symbol="월",             # 1월(원), 2월(별)로 구분
        hover_data=["학생명", "월"], # 마우스 올릴 때만 학생명 표시 (화면 깔끔)
        title="전체 학생 종합 지표 분포",
        template="plotly_white"
    )
    # 버블 안에 텍스트 제거하여 가독성 높임
    fig1.update_traces(marker=dict(sizemode='diameter', sizeref=0.5))
    ...
    fig2 = px.bar(
        ...
        x="학생명", y="성취도",    # X축: 학생, Y축: 성취도
        color="월",              # 1월/2월 색상 구분
        barmode="group",
        text="성취도",
        title="학생별 월별 성취도 점수 (1월 vs 2월)",
        template="plotly_white"
    )
